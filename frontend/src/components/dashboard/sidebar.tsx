...
export function SidebarNavHeaderTitle({ children, className }: SidebarGenericProps) {
    return <div className={cn(['text-xs uppercase text-muted-foreground ml-3 font-bold', className])}>{children}</div>;
}
...
export function SidebarNavLink({ children, className, href, active }: SidebarGenericProps<SidebarNavLinkProps>) {
    return (
        <Link href={href} className={cn(['flex items-center text-sm px-3 py-2 rounded-md ', active && 'bg-secondary', className])}>
            {children}
        </Link>
    );
}
